package articlelogic

import (
	"context"

	"github.com/krace-tx/emo_trash/app/rpc/article/article"
	"github.com/krace-tx/emo_trash/app/rpc/article/internal/svc"

	"github.com/zeromicro/go-zero/core/logx"
)

type GetDraftArticlesLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewGetDraftArticlesLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetDraftArticlesLogic {
	return &GetDraftArticlesLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
		Logger: logx.WithContext(ctx),
	}
}

// 查询草稿列表
func (l *GetDraftArticlesLogic) GetDraftArticles(in *article.GetDraftArticlesRequest) (*article.GetDraftArticlesResponse, error) {
	// todo: add your logic here and delete this line

	return &article.GetDraftArticlesResponse{}, nil
}
